...
# Bitcode was deprecated in Xcode 14; OpenSSL 3.x build errors if -fembed-bitcode is passed.
...
do
    ...
    # -target embeds LC_BUILD_VERSION so xcodebuild -create-xcframework can
    # distinguish iOS device from iOS simulator (both are arm64 otherwise).
    if [[ "${PLATFORM_NAME}" = "iphonesimulator" ]]; then
        ./Configure iossimulator-xcrun no-shared no-engine \
            CFLAGS="-target ${ARCH}-apple-ios14.0-simulator" \
            --openssldir=${PREFIX} --prefix=${PREFIX} > "${LOG}" 2>&1
    else
        ./Configure ios64-xcrun no-shared no-engine \
            CFLAGS="-target ${ARCH}-apple-ios14.0" \
            --openssldir=${PREFIX} --prefix=${PREFIX} > "${LOG}" 2>&1
    fi
